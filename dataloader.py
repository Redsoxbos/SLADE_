...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            # kitti, use_right, random>0.5면면 3,4번째 split 받아오고 아니면 0,1번째
            # 따라서 다른 데이터셋이어도 0,1번째로 그냥 image, depth path 받는다.(3,4번째 split이 애초에 없는거 같은데..?)
            ...
            image = Image.open(image_path)
             ...
             # synwoodscape면 depth npy로 부른 뒤 array로 바꾸고 아니면 Image로 부른다
            if self.args.dataset =='synwoodscape' or "kitti360":
            ...
            image = np.asarray(image, dtype=np.float32) / 255.0 # float으로 바꿔주고 255로 나눈다는것? 0~1사이로 바꿔주는거같은데.
            ...
            elif self.args.dataset == 'synwoodscape':
                pass
            ...
            if self.mode == 'online_eval':
                ...
                if has_valid_depth:
                    ...
                    if self.args.dataset == 'nyu':
                        depth_gt = depth_gt / 1000.0
                    elif self.args.dataset == 'synwoodscape' or 'kitti360':
                        pass           
                    else:
                        depth_gt = depth_gt / 256.0
