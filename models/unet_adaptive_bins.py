...
class UnetAdaptiveBins(nn.Module):
    def __init__(self, backend, n_bins=100, min_val=0.1, max_val=10, norm='linear'):
        ...
        luts = pd.read_pickle('/root/dev/AdaBins/LUTs_jongsung2.pkl')
        keys = list(luts.keys())
        self.angle_of_incidence = luts[keys[1]]['angle_maps'].reshape(-1)
        self.theta =luts[keys[1]]['theta'].reshape(-1)
    ...
    def forward(self, x, **kwargs):
        ...
        bin_edges = torch.cumsum(bin_widths, dim=1)   #[1,257]     
        #수정
        
        #return bin_edges, pred/(1/0.2419)
        ...
        #수정
        #pred = pred.reshape(out.shape[0],-1)
        #pred = torch.pow((torch.pow(pred/1+torch.tan(torch.tensor(self.angle_of_incidence,device=pred.device,requires_grad=False))*torch.cos(torch.tensor(self.theta,device=pred.device,requires_grad=False)),2)*torch.pow((torch.tan(torch.tensor(self.angle_of_incidence,device=pred.device,requires_grad=False))),2)+1),(1/2))
        #pred = pred.reshape(out.shape[0],-1,out.shape[2],out.shape[3])
