...
def eval(model, test_loader, args, gpus=None, ):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(sequential):
            ...
            final[final > args.max_depth_eval] = args.max_depth_eval # 수정
            

            #수정 
            #final[np.isinf(final)] = args.max_depth 원래 였음
            #final[np.isnan(final)] = args.min_depth 원래 였음.
            
            # factor이 무슨값인지 확인하고 수정
            if args.save_dir is not None:
                if args.dataset == 'nyu':
                ...
                #수정
                pred_path = os.path.join(args.save_dir, f"{impath}.npy")
                pred_path_raw = os.path.join(args.save_dir, f"{impath}_raw.npy")
                #pred = (final * factor).astype('uint16')
                np.save(pred_path, final)
                np.save(pred_path_raw, final_raw)
            ...
            valid_mask = np.logical_and(gt > args.min_depth, gt < args.max_depth_eval) #수정
            #수정
            ...
            ##수정
            #eval_mask = np.zeros(valid_mask.shape) 원래 없었다.
            ...
            #수정
            #valid_mask = np.logical_and(valid_mask, eval_mask) # 원래 있었다.
            #             gt = gt[valid_mask]
